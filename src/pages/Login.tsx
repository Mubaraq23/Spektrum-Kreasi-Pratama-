...
import {
  ...
  ChevronRight, Activity, Sun, Moon, ArrowRight, Shield, Database
} from 'lucide-react';
...
export function Login() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) return;
    setAuthLoading(true); setError(''); setResetSuccess('');
    try {
      if (isForgotPassword) {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 dark:bg-[#030612] font-sans">
      ...
      <div className="hidden lg:flex flex-col w-[52%] xl:w-[56%] relative overflow-hidden bg-gradient-to-br from-[#050c28] via-[#071030] to-[#050e30]">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-[0.15]">
          {[...Array(8)].map((_, i) => (
            <div key={i} className={`absolute top-0 bottom-0 w-px bg-blue-400/20 left-[${(i + 1) * 12.5}%]`} />
          ))}
          {[...Array(6)].map((_, i) => (
            <div key={i} className={`absolute left-0 right-0 h-px bg-blue-400/20 top-[${(i + 1) * 16.66}%]`} />
          ))}
        </div>
        ...
        <div className="relative z-10 flex flex-col h-full p-12 xl:p-16">
          ...
          <div className="py-16 max-w-md">
            ...
            <p className="text-slate-400 text-[15px] leading-relaxed font-medium mb-10 max-w-sm">
            ...
            <div className="space-y-3.5">
              {FEATURES.map(({ icon: Icon, text, color, bg }) => (
                <div key={text} className="flex items-center gap-3.5">
                  <div className={cn("w-8 h-8 rounded-xl flex items-center justify-center border border-white/10 shrink-0", bg)}>
          ...
          <div className="mt-auto pt-6 border-t border-white/10">
            <div className="flex flex-wrap items-center gap-4 text-[9px] font-bold uppercase tracking-widest text-slate-600 font-mono">
      ...
      {/* ══════════════════════════════════════
          RIGHT PANEL — Form
      ══════════════════════════════════════ */}
      <div className="flex-1 flex flex-col items-center justify-center p-5 sm:p-8 lg:p-14 relative overflow-y-auto">

        {/* Top bar */}
        <div className="absolute top-4 sm:top-6 left-4 sm:left-6 right-4 sm:right-6 flex items-center justify-between">
          <Link to="/"
            className="inline-flex items-center gap-2 text-[11px] font-bold uppercase tracking-widest text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors font-mono"
          >
        ...
        <div className="lg:hidden mb-8 mt-16">
          <Logo className="text-slate-900 dark:text-white h-8 max-w-[200px]" />
        </div>
        ...
        <div className="w-full max-w-[400px]">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-2 font-medium">{formSub}</p>
            </motion.div>
          </AnimatePresence>
          ...
          <AnimatePresence>
            {error && (
              <motion.div
                ...
                className="flex items-start gap-3 p-4 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900/60 rounded-2xl mb-5"
              >
                <AlertCircle className="w-4 h-4 text-red-500 shrink-0 mt-0.5" />
                <span className="text-[12px] text-red-700 dark:text-red-300 font-semibold leading-relaxed">{error}</span>
              </motion.div>
            )}
            {resetSuccess && (
              <motion.div
                initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                className="flex items-start gap-3 p-4 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-900/60 rounded-2xl mb-5"
              >
                <Check className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5" />
                <span className="text-[12px] text-emerald-700 dark:text-emerald-300 font-semibold leading-relaxed">{resetSuccess}</span>
              </motion.div>
            )}
          </AnimatePresence>
            ...
            >
              ...
              {!isForgotPassword && isRegistering && (
                <div className="relative group">
                  <Sparkles className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-blue-500 transition-colors pointer-events-none" />
                  <input
              ...
              {!isForgotPassword && (
                <div className="space-y-2">
                  <div className="relative group">
                    <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-blue-500 transition-colors pointer-events-none" />
                    <input
                    ...
                    <button
                      ...
                      className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors cursor-pointer p-1"
                    >
              ...
              {isForgotPassword ? (
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 text-[12px] text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 font-semibold transition-colors py-2 cursor-pointer"
                >
            ...
            <>
              <div className="relative my-7">
                ...
                <div className="relative flex justify-center">
                  <span className="px-4 bg-slate-50 dark:bg-[#030612] text-[10px] uppercase tracking-[0.3em] text-slate-400 font-bold font-mono">
              ...
              <button
                ...
                disabled={authLoading}
                className="w-full py-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-200 font-bold rounded-2xl flex items-center justify-center gap-3 hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-700 transition-all shadow-sm hover:shadow-md text-[13px] cursor-pointer"
              >
                <img
